...
export default class App extends React.Component {
  constructor(props) {
    super(props);
    this.position = 0;
    this.info = this.props.app.console ? JSON.parse(fs.readFileSync(`${ home }/Library/Application Support/ScoutKit/data/${ this.props.app.name.toLowerCase().replace(/[^\w\d]/g, '-') }/resources/info.json`, 'utf8')): undefined;
    this.state = {
      next: 'Next',
      value: 'next'
    };
  ...
  handleClick(target) {
    switch (target) {
      ...
      case 'new':
        ...
        break;
      case 'export':
        let manifest = JSON.parse(fs.readFileSync(`${ home }/Library/Application Support/ScoutKit/data/${ this.props.app.name.toLowerCase().replace(/[^\w\d]/g, '-') }/data/manifest.json`));
        for (let i = 0; i < manifest.length; i++) {
          fs.copySync(
            `${ home }/Library/Application Support/ScoutKit/data/${ this.props.app.name.toLowerCase().replace(/[^\w\d]/g, '-') }/data/${ manifest[i] }`,
            `${ this.props.app.export }/${ manifest[i] }`
          );
        }
        fs.copySync(
          `${ home }/Library/Application Support/ScoutKit/data/${ this.props.app.name.toLowerCase().replace(/[^\w\d]/g, '-') }/data/manifest.json`,
